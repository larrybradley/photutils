...
SEP (like SourceExtractor) adds ``1/12`` to both variances when the
determinant is below ``0.00694``. ``SourceCatalog`` raises each
eigenvalue to at least ``1/12`` and keeps the eigenvectors. Both give
a minor axis of ``1/sqrt(12)`` for a source that SEP flags, but SEP also
inflates the resolved major axis, and its determinant test misses a
source whose minor-axis variance is below ``1/12`` while its determinant
is not. The shape properties and centroid errors of singular sources
therefore differ by design. The degenerate scene has no single-pixel
segment because ``sep.extract`` returns invalid values for one when it
is given an existing segmentation map (SEP 1.4.1).
