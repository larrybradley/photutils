...
def make_stars(n_stars, *, fwhm=2.8, cutout_size=25, seed=0):
    ...
    The stars have random subpixel positions, fluxes spanning a factor
    of ten, and Poisson-like noise plus a constant background noise.
    ...
    data, params = make_psf_model_image((size, size), model, n_stars,
                                        model_shape=(cutout_size + 4,
                                                     cutout_size + 4),
                                        min_separation=separation,
                                        border_size=separation, seed=seed,
                                        flux=(1e4, 1e5))
...
def bench_stages(*, n_stars=400, oversampling=2, fwhm=2.8, cutout_size=25,
    ...
    print(f'\n== Build iteration stages (n_stars={n_stars}, '
          ...
          f'kernel={result.smoothing_kernel_shape}) ==')
