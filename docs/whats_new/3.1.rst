...
:class:`~photutils.aperture.ApertureStats` is regularized for
unresolved sources, because a source cannot have a second moment
smaller than ``1/12`` pixel**2, the variance of a uniform distribution
across a single pixel. Previously, following SourceExtractor, ``1/12``
was added to both diagonal elements when the covariance determinant was
below ``(1/12)**2``. That test exists to avoid a division by zero. It
misses a thin source that is unresolved along only its minor axis if its
major axis is long, it inflates the variance along a resolved axis, and
it is discontinuous.
...
NaN shape properties and the ``undefined_shape`` flag instead. A
source that is unresolved along both axes becomes
...
The covariance-derived shape properties of unresolved sources (e.g.,
``semimajor_axis``, ``semiminor_axis``, ``fwhm``, ``eccentricity``, and
the ``ellipse_c*`` coefficients) therefore change, as do the
``SourceCatalog`` quantities that are derived from the source shape,
such as the Kron aperture and the windowed centroid and its errors.
Single-pixel sources are unchanged. The ``SourceCatalog`` isophotal
centroid errors use the same approach. The error variance along each
principal axis of the error covariance is raised to the pixel-variance
floor, instead of adding the floor to both variances. This is applied to
any source whose error covariance is nearly singular, whatever the
source shape. These values no longer match SourceExtractor for
unresolved sources.
