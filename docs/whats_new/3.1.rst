...
The build now converges when at least a ``converged_fraction``
(default 0.95) of the successfully fitted stars have settled to the
requested ``center_accuracy``, instead of requiring every star. A few
spurious detections or contaminated cutouts whose centers never
settle previously forced the build to run to ``maxiters``. The
...
previous behavior. The star fitting step is also faster because the
ePSF spline interpolators are built once per iteration instead of
once per star.
