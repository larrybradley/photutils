...
returns a :class:`~photutils.segmentation.SourceCatalog`
object, which acts in part like a list of
:class:`~photutils.segmentation.LegacySourceProperties` objects, one for
each segmented source (or a specified subset of sources). An Astropy
`~astropy.table.QTable` of source properties can be generated using the
:meth:`~photutils.segmentation.SourceCatalog.to_table` method. Please
see :class:`~photutils.segmentation.LegacySourceProperties` for the
...
to :func:`~photutils.segmentation.source_properties`. The
output `~astropy.table.QTable` of source properties is
generated by the :class:`~photutils.segmentation.SourceCatalog`
:meth:`~photutils.segmentation.SourceCatalog.to_table` method.
Each row in the table represents a source. The columns
...
many properties that are calculated for each source:
...
Let's use the measured morphological properties to define
approximate isophotal ellipses for each source. Here we
define an `~photutils.aperture.EllipticalAperture` object
for each source using its calculated centroid positions
(`~photutils.segmentation.LegacySourceProperties.xcentroid` and
`~photutils.segmentation.LegacySourceProperties.ycentroid`)
, semimajor and semiminor axes lengths
(`~photutils.segmentation.LegacySourceProperties.semimajor_axis_sigma`
and
`~photutils.segmentation.LegacySourceProperties.semiminor_axis_sigma`)
, and orientation
(`~photutils.segmentation.LegacySourceProperties.orientation`):
...
:class:`~photutils.segmentation.LegacySourceProperties`, but a
subset of properties can also be specified (or excluded) in the
...
and input it into the :func:`~photutils.segmentation.source_properties`
function. When a total ``error`` is input, the
`~photutils.segmentation.LegacySourceProperties.source_sum_err`
property is calculated.
`~photutils.segmentation.LegacySourceProperties.source_sum` and
`~photutils.segmentation.LegacySourceProperties.source_sum_err` are the
...
:class:`~photutils.segmentation.LegacySourceProperties` class.
