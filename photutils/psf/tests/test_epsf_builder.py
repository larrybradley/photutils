...
def test_fit_stars_shares_spline_cache(epsf_test_data, monkeypatch):
    """
    The spline interpolators are built once per ePSF and shared by the
    model copies made for every star fit, so the number of spline
...
def test_resample_residuals_matches_per_star(epsf_test_data, oversampling):
    """
    The vectorized residual stack equals the per-star resampling,
    including masked pixels, out-of-grid pixels, and excluded stars.
    """
    stars = extract_stars(epsf_test_data['nddata'],
