...
def _suppress_alias_modes(data, oversampling, *, nu_pass=0.7, nu_stop=1.0):
    """
    Low-pass filter an oversampled ePSF above the input pixel
    sampling frequency.
    ...
    transition, and zero gain at and above ``nu_stop`` cycles per
    input pixel. Frequencies at integer cycles per input pixel are
    the zeros of the pixel response, so a pixel-integrated PSF has
    essentially no power there or above. However, those are exactly
    the frequencies at which the star-pixel sampling lattice aliases
    onto the oversampled grid, so noise at those frequencies can grow
    into a checkerboard pattern during the ePSF build iterations.
...
class EPSFBuildResults:
    ...
    converged : bool
        ...
        accuracy criterion. `True` if at least the ``converged_fraction``
        of the successfully fitted stars moved by less than the
        specified center accuracy between the final iterations.
    ...
    converged_fraction : float
        ...
        changed by less than ``center_accuracy`` in the final
        iteration. The build is converged when this fraction is at
        least the ``converged_fraction`` of the builder.
...
class EPSFBuilder:
    ...
    center_accuracy : float, optional
        ...
        iterations will stop when the centers of at least
    ...
    converged_fraction : float, optional
        ...
        to not prevent convergence. Set to 1.0 to require all stars to
        converge. The fraction achieved in the final iteration is
        reported in the ``converged_fraction`` attribute of the
    ...
    constrain_fluxes : bool, optional
        ...
        observed in multiple dithered images) to their mean value
        after each fitting iteration, in addition to constraining
        their centers to the same sky coordinate. This breaks the
        degeneracy between the flux of a star and its subpixel
        position caused by intra-pixel sensitivity variations, which
        would otherwise be absorbed into the ePSF. It assumes that the
        linked images have the same flux scale (e.g., the same
        exposure time and throughput). Set to `False` if the linked
        images have different flux scales. This parameter has no
        effect on stars that are not linked.
    ...
    After the residuals are combined and the ePSF is smoothed, power at
    and above one cycle per input pixel is removed along each
    ...
    sampling lattice aliases onto the oversampled grid. Without these
    two measures, noise in the ePSF grid from heterogeneous or
    contaminated stars can bias the fitted star centers toward
    ...
    def _resample_residual(self, star, epsf, *, out_image=None):
        ...
        pixel. Every star therefore contributes to every grid point
        that its cutout covers, regardless of its subpixel phase. For
        an oversampling factor of one along an axis, this reduces to
        ...
        # Each star pixel covers the grid points k with
        # x_over - os / 2 < k <= x_over + os / 2, which is exactly os
    ...
    def _build_epsf_step(self, stars, *, epsf=None):
        ...
        # onto the ePSF grid
        smoothed_data = _suppress_alias_modes(smoothed_data,
                                              self.oversampling)
        ...
        # are extrapolated (fill_value=None) rather than set to zero,
        # which would leave an all-zero row and column at that edge.
    ...
    def _check_convergence(self, stars, centers, fit_failed):
        ...
        by less than the configured center accuracy.
    ...
    def _warn_nonuniform_phases(stars, *, min_stars=32, pvalue=1.0e-6):
        ...
        phases. A strongly non-uniform distribution indicates that the
        fitted centers are biased toward particular phases, which
    ...
    def build_epsf(self, stars, *, epsf=None):
        ...
        - smoothing_kernel_shape: Smoothing kernel of the final iteration
