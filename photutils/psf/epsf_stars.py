...
class LinkedEPSFStar:
    ...
    def constrain_fluxes(self):
        ...
        The single flux is calculated as the mean of the fluxes of
        the linked stars. This assumes that the linked images have
        the same flux scale (e.g., the same exposure time and
        throughput). Averaging the fluxes across dithered images
        removes the pixel-phase dependence of the individual flux
        measurements caused by intra-pixel sensitivity variations,
        which would otherwise be absorbed into the ePSF (Anderson and
        King 2000).
        """
        if len(self._data) < 2:  # no linked stars
            return
