...
Provides the bodies of the circular, Kron, and flux-radius photometry
methods as module-level helper functions. The ``SourceCatalog`` methods
remain in `photutils.segmentation.catalog` as thin delegations to the
helpers in this module.

Each helper takes the host ``catalog`` instance as its first argument
and uses (often private) attributes on it for state (data, error, mask,
segmentation image, centroids, shape parameters, ``aperture_mask_method``,
``kron_params``, ``progress_bar``, etc.).
"""
...
# ------------------------------------------------------------------ #
# Cutout helpers used by aperture photometry
# ------------------------------------------------------------------ #

def _aperture_to_mask(catalog, aperture, **kwargs):
    """
...
def _make_aperture_data(catalog, label, x_centroid, y_centroid,
...
def _make_circular_apertures(catalog, radius):
    """
...
def _plot_circular_apertures(catalog, radius, ax, origin, **kwargs):
    """
    Plot circular apertures and return the list of patches.
    """
    apertures = _make_circular_apertures(catalog, radius)
    patches = []
...
def _circular_photometry(catalog, radius, name, overwrite):
    """
    ...
    flux, flux_err = _aperture_photometry(
        catalog, apertures, desc='circular_photometry', **kwargs)
...
def _make_elliptical_apertures(catalog, *, scale=6.0):
    """
...
def _measured_kron_radius(catalog):
    """
...
def _calc_kron_radius(catalog, kron_params):
    """
...
def _make_kron_apertures(catalog, kron_params):
    """
    ...
    return _make_elliptical_apertures(catalog, scale=scale)


def _plot_kron_apertures(catalog, kron_params, ax, origin, **kwargs):
    """
    ...
    if kron_params is None:
        ...
        apertures = _make_kron_apertures(catalog, kron_params)
...
def _aperture_photometry(catalog, apertures, *, desc='', **kwargs):
    """
    ...
    for label, aperture, bkg in zip(labels, apertures,
                                    catalog._local_background, strict=True):
...
def _calc_kron_photometry(catalog, *, kron_params=None):
    """
    ...
    if kron_params is None:
        ...
        kron_aperture = _make_kron_apertures(catalog, kron_params)
...
def _kron_photometry_table(catalog, kron_params, name, overwrite):
    """
    ...
    """
    kron_flux, kron_flux_err = _calc_kron_photometry(
        catalog, kron_params=kron_params)
    if catalog._data_unit is not None:
...
def _max_circular_kron_radius(catalog):
    """
...
def _flux_radius_optimizer_args(catalog):
    """
...
def _flux_radius(catalog, fraction, name, overwrite):
    """
    ...
    for flux_radius_args in args:
        ...
        # Try to find the root of _flux_radius_fcn, which is bracketed
        ...
        while max_radius > min_radius and found is False:
            try:
                ...
                root_result = root_scalar(
                    _flux_radius_fcn, args=fcn_args,
