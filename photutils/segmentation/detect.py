...
def _detect_sources_deblend(data, threshold, n_pixels, *, footprint,
    ...
    This is the deblending analogue of `_detect_sources`. It differs
    ...
    (the per-label cutout loop used by `_detect_sources` has a fixed
    per-label overhead that dominates for the small cutouts and the
    many calls made during deblending), and `None` is returned when
    fewer than two segments are found.
