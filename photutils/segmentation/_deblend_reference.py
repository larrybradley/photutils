...
kept for verification and debugging. It is not used in production.
It must track the compiled (Cython) semantics exactly, which is
enforced by the cross-implementation tests in
...
def _detect_sources_deblend(data, threshold, n_pixels, *, footprint,
                            segment_mask):
    """
    Detect sources for a single multithreshold level during
    deblending.

    This is the deblending analogue of
    `photutils.segmentation.detect._detect_sources`. It differs
    in that the detected segments keep their (possibly
    non-consecutive) label numbers from `~scipy.ndimage.label`, the
    small segments are removed with a bincount-based area filter
    (the per-label cutout loop used by ``_detect_sources`` has a
    fixed per-label overhead that dominates for the small cutouts
    and the many calls made during deblending), and `None` is
    returned when fewer than two segments are found.
...
class _SingleSourceDeblender:
    ...
    def compute_thresholds(self):
        ...
        therefore use the fallback mode, mirroring the sticky
        per-source mode fallback in the compiled pipeline.
