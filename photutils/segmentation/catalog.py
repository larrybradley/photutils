...
class SourceCatalog:
    ...
    **Scalar vs. Multi-source Catalogs**
    ...
    def _centroid_err_cov(self):
        ...
        # modified. The test is made on the error covariance itself,
        # so it needs no knowledge of the source shape.
    ...
    def _singular_covariance_mask(self):
        ...
        ``'undefined_shape'`` flag instead. The flag matches the
        equivalent aperture flag (see
    ...
    def _raw_covariance(self):
        ...
        tests it for singularity). Callers that modify the matrix in
        place must operate on a copy so the cached value is not
        corrupted.
        """
    ...
    def covariance(self):
        ...
        pixel**2 (to within floating-point rounding), the variance
        of a uniform distribution across a single pixel. For a source
        that is unresolved along an axis (e.g., a point-like or a very
        thin source), the variance along that axis is raised to ``1/12``
