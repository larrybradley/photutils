...
def _update_flags_docstring(func):
    ...
    is replaced with a bullet list generated from
    ``SEGMENTATION_FLAGS``.
...
class SourceCatalog:
    ...
    **Scalar vs. Multi-source Catalogs**
    ...
    def flags(self):
        ...
        The flags combine deblending-provenance flags carried by the
        input segmentation image with measurement-time flags computed
        from this catalog's ``data``, ``mask``, ``error``, and
        segmentation image. Use
        ...
        (``centroid_win_fallback``, ``centroid_quad_failed``), and
        Kron aperture geometry (``kron_undefined``,
        ``kron_minimum_radius``) flags derive from the detection
        catalog. The Kron photometry-loop flags
        (``kron_no_overlap``, ``kron_partial_overlap``,
        ``kron_masked_pixels``, ``kron_neighbor_pixels``,
        ``kron_uncorrected_pixels``), like the segment-level edge,
        mask, non-finite, and ``all_masked`` flags, are evaluated
        on this catalog's own inputs.

        Accessing ``flags`` computes the moment, covariance, centroid,
        and Kron-aperture properties if they have not already been
        computed (the results are cached and reused by those
        properties).
        ...
        # (convolved) data values have a zero net flux, so they are
        # also flagged here. The ``~(m00 > 0)`` form is defensive
        # against a non-finite net flux, which cannot currently occur
        # because the moment cutouts are zero-filled for masked and
        # non-finite pixels, but the form would still catch it.
        ...
        # Kron numerator or denominator is not positive. NaN
        # comparisons are False, so sources with an undefined Kron
        # radius are not flagged here (they are flagged as
        # kron_undefined instead). The Kron radii are measured by the
        # detection catalog, if input, so its kron_params define the
        # applied minimum.
    ...
    def _apply_centroid_win_fallback(self, xcen_win, ycen_win,
        ...
        result : `~numpy.ndarray`
            ...
            and covariance, and the fallback indicator as columns
            ``(x, y, var_x, var_y, cov_xy, fallback)``, shape
            ``(n_sources, 6)``. The ``fallback`` column is 1.0 for
            sources whose windowed centroid was reset to the isophotal
            centroid or is NaN (non-finite half-light radius), and 0.0
            otherwise.
        """
    ...
    def _centroid_win_results(self):
        ...
        centroid was reset to the isophotal centroid or is NaN, and
        0.0 otherwise.
    ...
    def _centroid_quad_results(self):
        ...
        for cutout, error_cutout, mask in zip(cutouts,
            ...
            # A source with no unmasked pixels has no peak; without
            # this guard the masked (zeroed) cutout below would return
            # the position of its first pixel as the "peak"
            if np.all(mask):
                results.append(nan_result)
                continue
    ...
    def _singular_covariance_flag_mask(self):
        ...
        This is the mask used for the ``'singular_covariance'`` flag.
        It matches the equivalent aperture flag (see
        ...
        eigenvalue of the raw covariance matrix) is less than
        ``1 / 12``. The determinant test alone misses elongated
        sources that are unresolved along only one axis. Sources with
        non-finite covariance are not flagged.
        """
        ...
        with warnings.catch_warnings():
            ...
            # lambda = tr/2 -/+ sqrt((tr/2)**2 - det). The
            # discriminant is non-negative for a real symmetric
            # matrix; clip tiny negative rounding to zero.
    ...
    def _make_aperture_data(self, label, x_centroid, y_centroid, aperture_bbox,
        ...
        needed to compute the aperture quality flags. Its
        ``segm_mask`` and ``uncorrected_mask`` values are `None` where
        they do not apply (i.e., for an ``aperture_mask_method`` of
        "none" and for a method other than "correct", respectively).
        """
    ...
    def _calc_kron_photometry(self, *, kron_params=None):
        ...
        for label, aperture, bkg in zip(labels, kron_aperture,
            ...
            # The box corners can have zero aperture weight, so
            # compare the number of nonzero-weight pixels within the
            # data to the total number in the aperture. The overlap is
            # partial only if at least one nonzero-weight pixel falls
    ...
    def _kron_photometry(self):
        ...
        kept in a single array that slices along the source axis. All
        of the flag bit values are exactly representable as floats.

        See the `SourceCatalog` ``aperture_mask_method`` keyword for
        options to mask neighboring sources.

        If the Kron aperture is `None`, then ``np.nan`` will be
        returned for the flux and flux error. This will occur where the
        source `centroid` position or elliptical shape parameters are
