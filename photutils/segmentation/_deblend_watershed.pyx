...
match ``skimage.segmentation.watershed`` (with ``compactness=0`` and
``watershed_line=False``), so the results are identical, but without
the per-call validation, padding, and cropping overhead of the
general-purpose function, which dominates for small cutouts.

The flood core runs without the GIL and uses no global mutable
state, so this module is safe to use from multiple threads,
...
def deblend_watershed(image, markers, mask, connectivity):
    ...
    This entry point is exported only for the pure-Python reference
    implementation in ``_deblend_reference`` and the
    cross-implementation tests; the production contrast loop calls
    the watershed core directly through ``deblend_source_contrast``.
...
def deblend_source_contrast(const data_t[:, ::1] data,
    ...
    output_arr = np.array(markers, dtype=np.int32, copy=True,
                          order='C')
