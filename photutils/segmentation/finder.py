...
class SourceFinder:
    ...
    contrast_method : {'basin', 'saddle'}, optional
        ...
        watershed basin. For ``'saddle'``, the fraction is the flux
        the source holds above the saddle level where it separates
        from its neighbors, equivalent to the SourceExtractor
        ``DEBLEND_MINCONT`` criterion. See
