...
class SourceFinder:
    ...
    contrast_method : {`None`, 'basin', 'saddle'}, optional
        ...
        basin. For ``'saddle'``, the fraction is the flux the source
        holds above the saddle level where it separates from its
        neighbors, equivalent to the SourceExtractor
        ``DEBLEND_MINCONT`` criterion. If `None` (default), the
        ``'basin'`` method is currently used; the default is planned
        to change to ``'saddle'`` in version 4.0. See
    ...
    def __init__(self, n_pixels, *, connectivity=8, deblend=True, n_levels=32,
                 ...
                 n_threads=1, n_processes=1, progress_bar=True):
