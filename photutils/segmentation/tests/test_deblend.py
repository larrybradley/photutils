...
def test_watershed_matches_skimage(connectivity):
    """
    Test that the deblending watershed kernel produces results
    identical to skimage.segmentation.watershed over randomized
    images, including integer-valued and constant images whose
    plateaus exercise the queue-age tie-breaking.
    """
...
def test_chunk_driver_matches_python_path(dtype, scene):
    ...
    the threshold computation, the mode fallbacks, the
    below-contrast marker removal, and the recorded warnings, for
    ...
    split, a constant source, and contrast values that trigger the
    batched removal, the one-at-a-time removal, the removal of all
    but one basin, and the removal path for sources with a negative
    minimum (which always removes one marker at a time).
    """
...
def python_saddle_markers(data, segment_mask, thresholds, n_pixels,
                          limit, footprint):
    """
    Reference implementation of the saddle-criterion marker
    selection.

    Builds the per-level component tree with per-level labeling and
    selects the markers with a global junction scan: a junction
    splits where at least two children with at least ``n_pixels``
    pixels each hold more flux above the junction level than
    ``limit``, and the markers are the passing children of splitting
    junctions that contain no deeper split.
...
def test_saddle_markers_match_reference(dtype, connectivity, scene,
    ...
    the pure-Python reference implementation, including nested
    splits, dissolving below-contrast siblings, the sinh fallback
    for negative minima, both connectivities, and float32 data.
    """
    ...
    markers_list, _ = deblend_markers_chunk(
        ...
        np.array([1], dtype=np.int64), y0, y1, x0, x1, n_pixels=5,
        connectivity=connectivity, n_levels=32, mode=1,
...
def test_nonposmin_fallback_sinh():
    ...
    The sinh spacing keeps the threshold levels concentrated near
    the source minimum, recovering a faint companion of a bright
    source that the linear spacing misses.
    """
...
def make_multipeak_source():
    ...
    The watershed basin flux fractions are approximately 0.061,
    0.062, 0.225, and 0.652, so the contrast keyword controls how
    many of the faintest basins fail the contrast criterion.
...
def test_contrast_removal(contrast, n_labels):
    ...
    next-faintest basin flux), the contrast=0.07 case removes the
    same two basins one at a time (their total is above the
    contrast), and the contrast=0.35 case removes all but one basin
    so that no deblending occurs.
    """
    data, segm = make_multipeak_source()
