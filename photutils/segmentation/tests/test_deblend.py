...
class TestDeblendSources:
    ...
    def test_flags_nonposmin_children(self):
        ...
        non-positive minimum data values carry both the deblended
        and fallback flags.
        """
        data = self.data.copy()
        data -= 20
...
def test_flags_fallback_without_deblending(relabel):
    ...
    # minimum (to trigger the nonposmin fallback). The middle label
    # is removed after detection so the remaining input labels (1
    # and 3) are non-consecutive, exercising the relabel-map
    # translation path when relabel=True.
