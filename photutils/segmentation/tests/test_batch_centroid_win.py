...
def _driver_inputs(cat, scene):
    ...
    skip = (nan_hl | np.isnan(xcen0)
            | np.isnan(ycen0)).astype(np.uint8)
...
def test_catalog_centroid_win(scene):
    # The frozen flags/centroid_win test suite is the primary
    # regression harness; this adds slicing and scalar cases against
    # the batch path on the shared scene
    cat = make_catalog(scene)
...
def test_detection_catalog(scene):
    # use_detcat fully delegates to the detection catalog; the batch
    # wiring must not break that indirection
...
def test_catalog_centroid_win_precomputed_slice(scene):
    # Slicing AFTER computing must equal computing AFTER slicing
