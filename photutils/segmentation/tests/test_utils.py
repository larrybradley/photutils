...
def test_mask_to_mirrored_value_return_uncorrected():
    """
    Test that return_uncorrected reports pixels whose mirror was
    outside the image.
    """
    center = (2.0, 2.0)
...
def test_mask_to_mirrored_value_return_uncorrected_masked():
    """
    Test that return_uncorrected reports pixels whose mirror is
    excluded by the ``mask`` keyword or is itself in ``replace_mask``.
    """
    center = (2.0, 2.0)
    data = np.arange(25.0).reshape(5, 5)
    replace_mask = np.zeros(data.shape, dtype=bool)
