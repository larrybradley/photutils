...
# Flags that intentionally share a name (and meaning, with the
# segment as the region) with APERTURE_FLAGS
...
def test_shared_aperture_names_intentional():
    ...
    A shared name must describe the same condition (with the
    segment as the region). Any new accidental overlap must either
    be renamed or added to the shared set deliberately.
    """
