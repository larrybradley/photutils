...
class TestSourceCatalogFlags:
    ...
    def test_edge_touch(self):
        """
        Test the edge_touch flag for a source segment touching the
        image boundary.
        """
        cat = SourceCatalog(self.data, self.segm)
    ...
    def test_masked_pixels(self):
        """
        Test the masked_pixels flag for input-masked pixels within
        the source segment.
        """
    ...
    def test_singular_covariance_elongated(self):
        ...
        ``(1/12)**2``, so it is flagged only by the minor-axis
        variance test.
        """
        data = np.zeros((11, 11))
        data[4, 2:9] = 0.04
    ...
    def test_centroid_flags_fully_masked_small(self):
        """
        Test that a fully-masked source smaller than the 3x3
        quadratic fit box flags both centroid failures.
        """
        data = np.zeros((21, 21))
        data[5:7, 5:7] = 10.0
    ...
    def test_kron_neighbor_pixels(self):
        """
        Test the kron_neighbor_pixels flag when a neighbor segment
        falls within the Kron aperture.
        """
    ...
    def test_kron_uncorrected_pixels(self):
        """
        Test the kron_uncorrected_pixels flag for neighbor pixels
        within the Kron aperture that could not be replaced by a
        mirrored value.
        """
        data, segm = self._make_deblended_pair()
        cat = SourceCatalog(data, segm, aperture_mask_method='correct')
        ...
        # neighboring sources are masked, not corrected, so no
        # pixels are left uncorrected
    ...
    def test_kron_neighbor_pixels_not_set(self):
        """
        Test that kron_neighbor_pixels is not set when the Kron
        aperture contains no neighboring source pixels.
        """
        cat = SourceCatalog(self.data, self.segm)
        assert not np.any(cat.flags
                          & SEGMENTATION_FLAGS.KRON_NEIGHBOR_PIXELS)
    ...
    def test_kron_masked_pixels(self):
        """
        Test the kron_masked_pixels flag for a masked pixel inside
    ...
    def test_kron_undefined(self):
        """
        Test the kron_undefined flag for a fully-masked source
        (whose Kron aperture is undefined).
        """
        mask = np.zeros(self.data.shape, dtype=bool)
        mask[self.segm.data == self.segm.labels[0]] = True
        cat = SourceCatalog(self.data, self.segm, mask=mask)
    ...
    def test_kron_undefined_oversized(self, circular):
        """
        Test the kron_undefined flag when the Kron photometry is
        skipped because the aperture is too large.
        """
        cat = SourceCatalog(self.data, self.segm)
    ...
    def test_kron_minimum_radius(self):
        """
        Test the kron_minimum_radius flag when the measured Kron
        radius is clipped to the kron_params minimum.
        """
        ...
        # [1.4318, 1.4058] against the default minimum of 1.4, a
        # margin of about 0.006 for the smaller value. A future
        # failure here likely indicates a fixture-tolerance drift
        # rather than a flag regression.
        cat2 = SourceCatalog(self.data, self.segm)
    ...
    def test_kron_minimum_circular_radius(self):
        ...
        """
        # A large minimum circular radius forces the circular
        # fallback for every source
        cat = SourceCatalog(self.data, self.segm,
    ...
    def test_kron_minimum_radius_detection_catalog(self):
        ...
        applied.
        """
        # The detection catalog does not apply its minimum, even
        # though the measured radii are below the requested minimum
    ...
    def test_kron_minimum_radius_two_element_params(self):
        """
        Test the kron_minimum_radius flag with a two-element
        kron_params (no minimum circular radius).
        """
        cat = SourceCatalog(self.data, self.segm,
                            kron_params=(2.5, 5.9))
        assert np.all(cat.flags
    ...
    def test_provenance_from_deblending(self):
        """
        Test that deblending provenance flags propagate into the
        catalog flags.
        """
        yy, xx = np.mgrid[0:101, 0:101]
    ...
    def test_flags_scalar_fresh(self):
        ...
        flags are cached computes a flags value that matches the
        parent catalog's flags.
        """
        cat = SourceCatalog(self.data, self.segm)
