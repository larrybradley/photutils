...
``n_pixels`` pixels removed) of the pixels above the threshold are
the candidate sources, and a marker is replaced by its components at
a higher level whenever it contains at least two of them. Computing
this directly requires one full labeling pass per level.

This kernel instead builds the quantized component tree once, by
adding pixels in decreasing level order to a union-find structure
and snapshotting the components at each populated level (levels
between populated values have identical components and are provably
no-ops in the per-level construction, so they need no snapshots).
The marker set is then derived by descending the tree: starting from
the components of the lowest level that has at least two components
with ``n_pixels`` or more pixels, each marker is replaced by its
sufficiently large components at the first higher level that
contains at least two of them. The resulting markers are identical
to the per-level construction, including the raster-scan ordering of
the marker labels.

The component-tree core runs without the GIL and uses no global
mutable state, so this module is safe to use from multiple threads,
...
cdef Py_ssize_t _markers_core(const int* qflat, Py_ssize_t ny,
    ...
    if result == 0 and use_saddle:
        ...
        # sufficiently large children hold more flux above the
        # junction level than the saddle limit; the markers are the
        # passing children of splitting junctions that contain no
        # deeper split themselves
    ...
    if result == 0 and n_final >= 2:
        # Paint each final marker by flood filling its component
        # from the recorded representative pixel; the marker regions
        # are disjoint, so each pixel is visited at most once
...
def make_deblend_markers(const int[:, ::1] quantized, int n_pixels,
    ...
    This single-source entry point is exported only for the
    pure-Python reference implementation in ``_deblend_reference``
    and the cross-implementation tests; production deblending goes
    through ``deblend_markers_chunk``.
...
cdef Py_ssize_t _source_markers(const data_t* data, const segm_t* segm,
    ...
    pixels excluded), applies the exponential-mode fallback to sinh
    for non-positive minima and, for the basin criterion, the
    too-many-markers fallback to linear mode (recording both in
    ``flag``), quantizes the cutout, and runs the component-tree
    core with the selected contrast criterion. Returns the number of
    markers (0 means the source does not split) or -1 if a memory
    allocation failed.
    """
    ...
    if mode == MODE_EXPONENTIAL and smin <= 0:
        ...
        # minima; fall back to sinh mode, which keeps the levels
        # concentrated near the source minimum
    ...
    n_markers = _markers_core(q, ny_c, nx_c, n_pixels, conn8, posimg,
                              thresholds, saddle_limit, use_saddle,
                              parent, size, fsum, added, stamp,
    ...
    # thresholds). With the saddle criterion the markers are already
    # contrast-selected and there is no iterative watershed, so no
    # fallback is needed.
    if (not use_saddle and n_markers > max_markers
            and use_mode != MODE_LINEAR):
...
def deblend_markers_chunk(const data_t[:, ::1] data,
                          ...
                          int max_markers=200, saddle_limits=None):
    """
    ...
    max_markers : int, optional
        ...
        mode (ignored with the saddle criterion).
    ...
    saddle_limits : 1D float64 `~numpy.ndarray` or `None`, optional
        ...
        junction of the component tree splits only where at least
        two sufficiently large components each hold more flux above
        the junction level than this per-source limit (the contrast
        times the total source flux).
    ...
    for isrc in range(n_src):
        ...
        with nogil:
            n_markers = _source_markers(
                ...
                saddle_limit, use_saddle,
                &thresholds_mv[0], &q_mv[0], posimg, fsum,
                &parent_mv[0], &size_mv[0], &added_mv[0],
                &stamp_mv[0], &node_of_root_mv[0], &order_mv[0],
                &flood_mv[0], &markers_mv[0], &flags_mv[isrc])
        if n_markers < 0:
            raise MemoryError
        if n_markers >= 2:
            n_tot = ny_c * nx_c
