...
and snapshotting the components at each populated level. The marker
set is then derived by descending the tree: starting from the
components of the lowest level that has at least two components with
``n_pixels`` or more pixels, each marker is replaced by its
