...
cdef Py_ssize_t _markers_core(const int* qflat, Py_ssize_t ny,
    ...
    if result == 0 and n_final >= 2:
        ...
        if result == 0:
            ...
            # pixels (an insertion sort; the first pixels are distinct),
            # matching the ordering that per-level labeling would
            # produce.
            for m in range(n_final):
                order_rank[m] = m
...
cdef Py_ssize_t _source_markers(const data_t* data, const segm_t* segm,
    ...
    pixels excluded), applies the exponential-mode fallback to
    sinh for non-positive minima and, for the basin criterion, the
    too-many-markers fallback to linear mode (recording both in
    ``flag``), quantizes the cutout, and runs the component-tree core
    with the selected contrast criterion. Returns the number of markers
