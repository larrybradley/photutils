...
def deblend_sources(data, segmentation_image, n_pixels, *, labels=None,
    ...
    segment_image : `~photutils.segmentation.SegmentationImage`
        ...
        empty if no mode fallbacks occurred. The ``flags`` attribute
        of the returned segmentation image records per-source
        deblending provenance; see
...
def _make_flags_map(deblend_label_map, nonposmin_labels, n_markers_labels,
    ...
    are set on the children of affected parents, or on the parent's
    own (possibly relabeled) output label if it did not split.

    Parameters
    ----------
    deblend_label_map : dict
        Mapping of input parent labels to arrays of output child
        labels, in the final (post-relabel) label frame.
    ...
    relabel_map : `~numpy.ndarray` or `None`
        The relabeling map applied to the deblended image, or `None`
        if no relabeling was applied.
    ...
    for input_labels, bit in fallbacks:
        for label in input_labels:
            ...
            if label in deblend_label_map:
                ...
                # The source did not split; translate its input
                # label to the output label frame
