...
def deblend_sources(data, segmentation_image, n_pixels, *, labels=None,
    ...
    contrast : float, optional
        ...
        deblend sources with a 7.5 magnitude difference. The
    ...
    contrast_method : {`None`, 'basin', 'saddle'}, optional
        ...
        territory assigned to the source; basins below the contrast
        are removed iteratively (faintest first, in batches when
        provably equivalent) and their territory is re-flooded. For
        ``'saddle'``, the fraction is the flux the source holds
        above the saddle level where it separates from its
        neighbors, evaluated once during marker construction with no
        iteration; this is equivalent to the `SourceExtractor`_
        ``DEBLEND_MINCONT`` criterion and measures the significance
        of the peak itself, independently of how much envelope
        territory it would inherit. Because ``'saddle'`` needs only
        a single watershed pass, it is never slower than ``'basin'``
        and is substantially faster when many below-contrast basins
        would otherwise be removed iteratively. Note that the two
        methods measure different fluxes, so the same ``contrast``
        value selects sources differently: the saddle flux excludes
        all territory below the saddle and is therefore smaller than
        the basin flux, making ``'saddle'`` stricter for faint peaks
        on bright envelopes. If `None` (default), the ``'basin'``
        method is currently used; the default is planned to change
        to ``'saddle'`` in version 4.0.
...
def _deblend_sources_chunk(data, segm_data, driver_data, driver_segm,
    ...
    # The saddle criterion selects the markers against the total
    # source fluxes, so those are computed up front; the basin
    # criterion needs them only for the sources that split
    use_saddle = deblend_params.contrast_method == 'saddle'
    saddle_limits = None
    source_stats = {}
    if use_saddle:
        saddle_limits = np.empty(len(labels), dtype=np.float64)
        for index, (label, slc) in enumerate(zip(labels, slices,
                                                 strict=True)):
            values = data[slc][segm_data[slc] == label]
            source_stats[index] = (float(nansum(values)),
                                   float(nanmin(values)))
            saddle_limits[index] = (deblend_params.contrast
                                    * source_stats[index][0])
    ...
    for index, (label, slc, markers, flag) in enumerate(
        ...
        # The total source flux and minimum are computed with the
        # same reductions as the per-source Python path and passed
        # to the compiled contrast loop. With the saddle criterion,
        # the markers are already contrast-selected, so the basin
        # removal is disabled by an unreachable contrast.
        if use_saddle:
...
class _SingleSourceDeblender:
    ...
    def make_marker_segment(self, segment_lower, segment_upper):
        ...
        # Count the upper-level children of each lower-level label
        # from the unique (lower, upper) label pairs, encoded as a
        # combined integer key
        ...
        # the parent mask that are below the upper threshold are
        # unset. Single-child parents are kept as is (maximizing the
        # marker size).
    ...
    def _remove_faint_markers(self, markers, labels, flux_frac):
        ...
        values are all nonnegative, the largest batch of the
        faintest markers is removed whose total flux fraction is
        below both the contrast and the next-faintest marker flux
        fraction. Removing such a batch in one step is equivalent to
        removing its markers one at a time: every batch member stays
        below the contrast no matter how the flux of the other
        removed members is redistributed (their total is below the
        contrast), and markers outside the batch can only become
        brighter, so the faintest below-contrast marker always lies
        inside the batch until the batch is exhausted.

        Faint markers that do not fit in such a batch are removed
        one at a time because several faint sources could combine to
        meet the contrast criterion.
