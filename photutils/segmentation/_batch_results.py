...
class BatchFluxRadiusArgs(NamedTuple):
    ...
    The cleaned cutouts of all sources are packed into one buffer. A
    source that cannot have a solution has a zero pixel count.
    """
    ...
    """
    The ``(n_sources, 4)`` grid edges ``(xmin, xmax, ymin, ymax)`` of
    each cutout, relative to the source centroid.
    """
    ...
    use_exact: int
    """
    Whether the root-find uses exact overlap fractions (1) or subpixel
    sampling (0).
    """
