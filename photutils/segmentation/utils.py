...
def _mask_to_mirrored_value(data, replace_mask, xycenter, *, mask=None,
    ...
    return_uncorrected : bool, optional
        ...
        pixels that could not be replaced by a mirrored value (i.e.,
        the mirror pixel was outside the image, in ``mask``, or itself
        in ``replace_mask``) and were set to zero instead.

    Returns
    -------
    result : 2D `~numpy.ndarray`
        A 2D array with replaced masked pixels. If
        ``return_uncorrected`` is `True`, then a tuple of
        ``(result, uncorrected)`` is returned instead, where
        ``uncorrected`` is a 2D bool `~numpy.ndarray` that is `True`
        for the ``replace_mask`` pixels that were set to zero because
        their mirrored value was unavailable.
    """
