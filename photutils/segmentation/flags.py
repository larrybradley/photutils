...
class _SegmentationFlags(FlagRegistry):
    ...
    segmentation flag definitions, including bit values, names, and
    descriptions. The same flag definitions are used by
    `~photutils.segmentation.SegmentationImage`,
    `~photutils.segmentation.deblend_sources`, and
    `~photutils.segmentation.SourceCatalog`, so a given bit always
    ...
    segment as the measurement region, use the same name as the
    aperture flag. The bit values are independent between packages;
    always decode a flag column with the decoder of the package
    that produced it. Flags describing the Kron aperture use a
    ``kron_`` name prefix.
...
def _update_decode_docstring(func):
    ...
    replaced with a bullet list generated from ``SEGMENTATION_FLAGS`` (see
...
def decode_segmentation_flags(flags, *, return_bit_values=False):
    ...
    """
    Decode segmentation bitwise flag values into individual
    components.

    This function takes integer flag values from segmentation
    operations and returns a list of human-readable names of the
    conditions that were detected. This is useful for understanding
    what problems were encountered without needing to manually
    ...
    flags : int or array-like of int
        ...
        represents a specific condition that was detected when
