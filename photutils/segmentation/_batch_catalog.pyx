...
per-source outputs without materializing per-source cutouts or making
...
cdef void _centroid_win_source(const double *data, const double *error,
    ...
    radius ``4 * sigma``, with masked pixels contributing zero and
    neighbor-source pixels excluded or mirror-corrected per
    ...
    # moment/error pass (the window may go empty on a later
    # iteration, in which case the previous iteration's window is
    # what the Python implementation reused)
...
def batch_centroid_win(const double[:, ::1] data, *,
    ...
    For each source, an iterative Gaussian-weighted centroid is
    computed within a binary circular window of radius ``4 * sigma``
    centered on the current centroid estimate. The iteration, the
    integer bounding-box arithmetic, the per-pixel masking, and the
    final windowed second-order moments and raw error sums replicate
    the previous per-source Python implementation exactly.
