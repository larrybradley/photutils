...
def batch_aperture_sums(const double[:, ::1] data, const double[:, ::1] error,
    ...
    params_per_source : 2D ndarray of float64 (C-contiguous) or `None`
        ...
        vector (which must then be `None`). Only the circle
        (``k`` = 1, ``(r,)``) and ellipse (``k`` = 3, ``(a, b,
        theta)``) shapes are supported, and ``emit_sum`` must be zero.
        The bounding-box half-extents are computed from each source's
        own parameters, so the ``ext_x`` and ``ext_y`` inputs are
        ignored.
    ...
    flag_counts : 2D ndarray of intp
        ...
        * ``FLAG_COL_UNCORRECTED_MASKED``: the number of those masked
        ...
        applied, so they are counted in the ``FLAG_COL_SEG_MASKED`` and
        ``FLAG_COL_UNCORRECTED_MASKED`` columns instead of the
    ...
    weights_out : 1D ndarray of uint8
        A per-source 0/1 indicator of whether the aperture has one or
        more nonzero-fraction pixels outside the data. This is the
        precise outside-weight test, computed only for the sources
        whose bounding box is clipped by a data edge (the
        ...
        for k in range(n_src):
            ...
            for iy in range(sy0, sy1):
                ...
                for ix in range(sx0, sx1):
                    ...
                    # For a clipped bounding box, the pixel loop also
                    # covers the pixels outside the data. Such a pixel
                    # with a nonzero fraction is the precise
                    ...
                    if has_mask:
                        ...
                        if mbits != 0:
                            ...
                            # masked neighbor-segment pixels (and
                            # their mirror availability) here for
                            # callers that treat the mask and neighbor
                            # overlays independently. The helper is
                            # called only for its counter side
                            # effects; its return value and resolved
