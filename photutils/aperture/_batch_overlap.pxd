...
cdef inline bint _resolve_seg_pixel(const Py_ssize_t *segmentation,
    ...
    It is normally called on unmasked pixels, but a caller may also
    call it on a masked pixel purely for its counter side effects, in
    which case its return value is ignored.
