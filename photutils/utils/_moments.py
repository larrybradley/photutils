...
def covariance_min_eigval(covariance, *, determinant):
    ...
    difference of the two eigenvalues. For a symmetric matrix ``[[a,
    b], [b, d]]`` it is ``hypot((a - d) / 2, b)``, which is never
    negative and does not overflow for large variances.
...
def is_invalid_covariance(covariance, *, determinant):
    ...
    mask : `~numpy.ndarray`
        ...
        its trace is negative or if its determinant is negative
        infinity while its trace is finite.
    """
    # Ignore floating-point errors from non-finite values in the
    # covariance and from the overflow of huge variances
    with np.errstate(over='ignore', invalid='ignore'):
...
def is_singular_covariance(covariance, *, determinant):
    ...
    This covers point-like sources, where both axes are unresolved, and
    thin sources that are unresolved along only one axis. A test of the
    determinant against ``PIXEL_VARIANCE**2`` is implied for a symmetric
    matrix, because a determinant below that value requires an
    eigenvalue below ``PIXEL_VARIANCE``. A covariance matrix that is
    not positive semidefinite (see `is_invalid_covariance`) is not
    flagged. It does not describe a shape, so it is set to NaN instead
    of being regularized. The flagged sources are therefore exactly the
    ones that `regularize_covariance` modifies and keeps finite.
...
def floor_covariance_eigvals(covariance, *, minimum):
    ...
    The variance along each principal axis is floored independently, so
    the orientation is unchanged and an axis whose variance already
