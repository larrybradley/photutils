...
- ``photutils.segmentation``
  ...
  - Significantly improved the performance of the ``SourceCatalog``
    ...
    by computing all sources in a single call into compiled code. The
    moment-based properties (e.g., ``covariance``, ``fwhm``,
    ``orientation``, and the centroid errors) are about 10 times
    faster, and the ``flags`` attribute and the default ``to_table()``
    output are about 3 times faster for large catalogs. Progress bars
    are no longer displayed for these calculations. [#xxxx]
